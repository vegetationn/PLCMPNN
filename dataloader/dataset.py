...
if __name__ == "__main__":
    ...
    # bace
    data = pd.read_csv(f'{data_path}/bace.csv', sep=',')
    data = data.fillna(-1.0)
    smiles = data['mol'].values.tolist()
    labels = data['Class'].values.tolist()
    build_dgl_graph(smiles, labels, result_path, mode="bace")
    ...
    # hiv
    data = pd.read_csv(f'{data_path}/hiv.csv', sep=',')
    data = data.fillna(-1.0)
    smiles = data['smiles'].values.tolist()
    labels = data['HIV_active'].values.tolist()
    build_dgl_graph(smiles, labels, result_path, mode="hiv")
    ...
    data = data.fillna(-1.0)
    smiles = data['smiles'].values.tolist()
    labels = data['u0_atom'].values.tolist()
    build_dgl_graph(smiles, labels, result_path, mode="qm7")

    # qm8
    data = pd.read_csv(f'{data_path}/qm8.csv', sep=',')
    data = data.fillna(-1.0)
    smiles = data['smiles'].values.tolist()
    labels = data.iloc[:, 1:].values.tolist()
    build_dgl_graph(smiles, labels, result_path, mode="qm8")

    # qm9
    data = pd.read_csv(f'{data_path}/qm9.csv', sep=',')
    data = data.fillna(-1.0)
    smiles = data['smiles'].values.tolist()
    labels = data.iloc[:, 2:].values.tolist()
    build_dgl_graph(smiles, labels, result_path, mode="qm9")
